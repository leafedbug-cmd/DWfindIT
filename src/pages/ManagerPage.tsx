  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        // 2) fetch profiles for listed users (if table exists)
        ...
        if (userIds.length) {
          ...
          if (!profErr && profData) {
            ...
            if (active) setProfiles(byId);
          }
          // if profiles table is missing, we silently fall back to user_id
        }
      } catch (e: any) {
          ...
          <div className="space-y-3">
            ...
            {grouped.size === 0 && (
              <div className="p-4 text-gray-600 bg-white border border-gray-200 rounded-lg">
                No employee lists found for this store yet.
              </div>
            )}
          </div>
        )}
