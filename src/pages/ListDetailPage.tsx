...
import { Trash2, Plus, Package, Hash, FileText, FileSpreadsheet, Search, Minus } from 'lucide-react';
...
const ItemDetails: React.FC<{ item: ListItem }> = ({ item }) => {
    ...
    if (item.item_type === 'part' && item.parts) {
        return (
            <>
                <Hash className="h-8 w-8 text-gray-400 mr-4 flex-shrink-0" />
                <div className="flex-grow">
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col pb-32">
      ...
      <main className="flex-1 p-4 space-y-4">
        <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-200 grid grid-cols-1 gap-3">
            ...
            <button onClick={() => { setSearchError(null); setIsSearchOpen(true); }} className="flex items-center justify-center gap-2 text-sm bg-blue-600 text-white px-4 py-2 rounded-lg shadow-sm hover:bg-blue-700">
