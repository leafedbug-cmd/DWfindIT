...
import { generateCSV, generatePDF } from '../utils/export'; // Assuming you have this file
...
export const ListDetailPage: React.FC = () => {
  ...
  // Find the current list from the lists array
  ...
  const handleExportCSV = () => {
    if (currentList && items) {
      ...
      link.setAttribute('download', `${currentList.name.replace(/\s+/g, '_')}.csv`);
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col pb-16">
      ...
      <main className="flex-1 p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2">
            <button onClick={handleExportCSV} className="text-sm bg-blue-500 text-white px-4 py-2 rounded-lg shadow-sm">Export CSV</button>
            <button onClick={handleExportPDF} className="text-sm bg-red-500 text-white px-4 py-2 rounded-lg shadow-sm">Export PDF</button>
        </div>
