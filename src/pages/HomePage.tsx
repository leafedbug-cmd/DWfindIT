...
export const HomePage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 flex flex-col pb-16">
      ...
      <main className="flex-1 p-4 text-gray-900 dark:text-gray-100">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <button onClick={handleQuickScan} className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 hover:shadow-md transition-shadow flex flex-col items-center">
              <Scan className="h-8 w-8 text-orange-600 mb-2" />
              <span className="font-medium text-gray-900 dark:text-gray-100">Quick Scan</span>
              <span className="text-sm text-gray-500 dark:text-gray-300 text-center">Scan to view part info</span>
            </button>
      ...
      {isQuickScanning && ( <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4"> <div className="bg-white dark:bg-slate-900 rounded-lg max-w-md w-full p-6"> <div className="flex justify-between items-center mb-4"> <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Quick Scan</h3> <button onClick={closeQuickScan} className="text-gray-400 hover:text-gray-600 dark:text-gray-300 dark:hover:text-gray-100"> <X className="h-6 w-6" /> </button> </div> <div id="quick-scanner" className="w-full h-64 border-2 border-gray-300 dark:border-slate-700 rounded-lg mb-4" style={{ minHeight: '256px', backgroundColor: '#f9fafb' }}></div> {scanError && <div className="mb-4 p-3 bg-red-100 text-red-700 rounded">{scanError}</div>} {quickScanResult && ( <div className="mb-4 p-4 bg-green-100 dark:bg-green-900/50 rounded text-gray-900 dark:text-gray-100"> <h4 className="font-semibold text-green-800 dark:text-green-200">Part Found!</h4> <p><strong>Part:</strong> {quickScanResult.part_number}</p> <p><strong>Bin:</strong> {quickScanResult.bin_location}</p> </div> )} </div> </div> )}
      <BottomNav />
    </div>
  );
};
